...
class FSRS6APPolicyTests(unittest.TestCase):
    def test_weight_clipper_uses_expected_bounds(self) -> None:
        low = [-100.0] * 21
        high = [1000.0] * 21

        self.assertEqual(clip_fsrs6_ap_weights(low)[0], 0.001)
        self.assertEqual(clip_fsrs6_ap_weights(low)[19], 0.01)
        self.assertEqual(clip_fsrs6_ap_weights(low)[20], 0.1)
        self.assertEqual(clip_fsrs6_ap_weights(high)[0], 100.0)
        self.assertEqual(clip_fsrs6_ap_weights(high)[16], 6.0)
        self.assertEqual(clip_fsrs6_ap_weights(high)[20], 0.8)
        self.assertEqual(FSRS6_AP_WEIGHT_BOUNDS[19], (0.01, 0.8))

    def test_zero_delta_decodes_to_clipped_base_weights(self) -> None:
        decoded = decode_weight_delta(DEFAULT_FSRS6_WEIGHTS, [0.0] * 21)

        self.assertEqual(decoded, clip_fsrs6_ap_weights(DEFAULT_FSRS6_WEIGHTS))
    ...
    def test_policy_round_trips_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            loaded = FSRS6APPolicy.from_json(path)
    ...
    def test_zero_delta_scheduler_matches_fsrs6_scheduler(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ap = FSRS6APScheduler(policy_json=path)

            fsrs_interval, fsrs_state = fsrs.init_card(_view(None), 3, 0.0)
            ap_interval, ap_state = ap.init_card(_view(None), 3, 0.0)
            self.assertAlmostEqual(ap_interval, fsrs_interval, places=9)
            ...
            for elapsed, rating in zip([1.0, 4.0, 12.0], [3, 4, 1]):
                ...
                ap_interval, ap_state = ap.schedule(
                    _view(ap_state), rating, elapsed, elapsed
                )
                self.assertAlmostEqual(ap_interval, fsrs_interval, places=9)
                self.assertAlmostEqual(ap_state["s"], fsrs_state["s"], places=9)
                self.assertAlmostEqual(ap_state["d"], fsrs_state["d"], places=9)
