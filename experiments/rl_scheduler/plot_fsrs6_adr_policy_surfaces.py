...
def _write_user_plot(
    *,
    ...
    fig.update_layout(
        title=(
            f"FSRS6 ADR retention policy surfaces: user {user_id}, lambda={lambda_text}"
        ),
        scene={
    ...
    output_path = (
        out_dir
        / f"fsrs6_adr_policy_surfaces_user_{user_id}_lambda_{_lambda_label(lambda_value)}.html"
    )
