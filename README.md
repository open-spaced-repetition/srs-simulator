...
- `run_sweep_users_batched.py` runs LSTM-only retention sweeps in batched vectorized mode on a single device; use `--batch-size` to control how many users are simulated in parallel.
