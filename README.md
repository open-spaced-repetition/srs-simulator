...
- Plots: for baseline schedulers (default Anki-SM-2 and Memrise), it computes each user's equivalence-target DR by interpolating the target scheduler (FSRS-6 by default, or FSRSv3 via `--equiv-report fsrs3`) points to match the baseline memorized cards (average, all days), then compares memorized cards per minute (average) distributions along with per-user differences and ratios.
