...
FEATURE_VERSION_LOG_POLY = "fsrs6_adr_direct_log_poly_v1"
FEATURE_VERSION_LOG_LINEAR = "fsrs6_adr_direct_log_linear_v1"
...
class FSRS6ADRDirectPolicy:
    ...
    @classmethod
    def from_json(cls, path: str | Path) -> FSRS6ADRDirectPolicy:
        policy_path = Path(path)
        with policy_path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
        if not isinstance(raw, dict):
            raise ValueError(
                f"FSRS6 ADR Direct policy {policy_path} must be a JSON object."
            )
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.coefficients) != expected_count:
            raise ValueError(
                f"FSRS6 ADR Direct policy expects {expected_count} coefficients for "
...
def _feature_count(feature_version: str) -> int:
    try:
        ...
        raise ValueError(
            f"Unsupported FSRS6 ADR Direct feature_version {feature_version!r}; "
...
def _default_title(feature_version: str) -> str:
    if feature_version == FEATURE_VERSION_LOG_LINEAR:
        return "FSRS6 ADR Direct log linear"
    _feature_count(feature_version)
    return "FSRS6 ADR Direct log polynomial"
