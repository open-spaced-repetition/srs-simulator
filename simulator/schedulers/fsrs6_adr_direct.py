...
class FSRS6ADRDirectScheduler(Scheduler):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.policy = FSRS6ADRDirectPolicy.from_json(policy_json)
        weights = resolve_fsrs6_weights(fsrs_weights)
        if len(weights) != 21:
            raise ValueError("FSRS6ADRDirectScheduler expects 21 FSRS-6 weights.")
...
@dataclass
class FSRS6ADRDirectVectorizedState:
    s: "torch.Tensor"
    d: "torch.Tensor"


class FSRS6ADRDirectVectorizedSchedulerOps:
    def __init__(
        self,
        scheduler: FSRS6ADRDirectScheduler,
        *,
        device: "torch.device",
        dtype: "torch.dtype",
    ) -> None:
    ...
    def init_state(self, deck_size: int) -> FSRS6ADRDirectVectorizedState:
        s = self._torch.full(
        ...
        return FSRS6ADRDirectVectorizedState(s=s, d=d)

    def review_priority(
        self,
        state: FSRS6ADRDirectVectorizedState,
        idx: "torch.Tensor",
...
@dataclass
class FSRS6ADRDirectBatchState:
    s: "torch.Tensor"
    d: "torch.Tensor"


class FSRS6ADRDirectBatchSchedulerOps:
    PRIORITY_MODES = FSRS6ADRDirectScheduler.PRIORITY_MODES
    ...
    def __init__(
        ...
        *,
        ...
        if weights.ndim != 2 or weights.shape[1] != 21:
            raise ValueError(
                "FSRS6ADRDirectBatchSchedulerOps expects weights shape (users, 21)."
            )
        self._torch = torch
        self.device = device
        self.dtype = dtype
        ...
        if coefficients is None:
            ...
            if coefficients.ndim != 2 or coefficients.shape != expected_shape:
                raise ValueError(
                    "FSRS6 ADR Direct batch coefficients must have shape "
    ...
    def init_state(self, user_count: int, deck_size: int) -> FSRS6ADRDirectBatchState:
        s = self._torch.full(
        ...
        return FSRS6ADRDirectBatchState(s=s, d=d)

    def review_priority(
        self, state: FSRS6ADRDirectBatchState, elapsed: "torch.Tensor"
    ) -> "torch.Tensor":
    ...
    def update_review(
        self,
        state: FSRS6ADRDirectBatchState,
