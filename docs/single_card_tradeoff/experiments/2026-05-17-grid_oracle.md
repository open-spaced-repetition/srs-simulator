...
This report uses the structured same-target time saved AUC output from the exact oracle comparison run.
...
The stationary finite exact policy gives up a small amount of time-saved performance and coverage versus the unrestricted finite-horizon table, but removes the remaining-time policy input.
