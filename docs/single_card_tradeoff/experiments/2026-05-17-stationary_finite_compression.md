...
How far can the stationary finite distill be compressed while preserving relative time saved and span coverage?
...
| variant | weights | params | relative_time_saved | coverage |
| --- | --- | --- | --- | --- |
...
| variant | family | arch | params | epochs | agreement | relative_time_saved | coverage |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
After aligning epochs and evaluation seeds, the 316-parameter `residual:8:1` student recovers frontier span and remains competitive with larger students. The 216-parameter `residual:6:1` student also keeps span coverage, but with weaker relative time saved in this rerun. In the quick sub-216 sweep, the best time-saved row is `residual:5:1` at 172 parameters, 20.00% +/- 0.34% relative time saved, and 95.30% +/- 0.29% coverage. The best coverage row is `quadratic` at 110 parameters, 10.05% +/- 0.63% relative time saved, and 99.36% +/- 0.17% coverage. Both are weaker than 22.10% +/- 0.33% relative time saved and 98.37% +/- 0.17% coverage for `residual:6:1`. The separate stationary finite epoch extension report tests whether longer training changes the sub-216 conclusion.
