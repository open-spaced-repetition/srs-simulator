...
are implemented. `train-overfit` requires `training.command_template`, executes
it once per training user and lambda value, and validates scheduler policy
metadata under `{output_dir}`. `sweep` requires `sweep.command_template`,
executes it once per trained artifact, and validates JSONL logs under
`{output_dir}`. `pareto` requires `pareto.command_template` and validates Pareto
JSON/PNG artifacts under `{output_dir}`. Treat later stages as unavailable until
they return machine-readable evidence and enforce their gates. `all` is
fail-fast and writes `all_summary.json`.
