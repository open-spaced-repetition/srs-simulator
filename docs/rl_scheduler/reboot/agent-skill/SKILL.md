...
implemented. `train-overfit` requires `training.command_template`, executes it
once per training user and lambda value, and validates scheduler policy metadata
under `{output_dir}`. `sweep` requires `sweep.command_template`, executes it once
per trained artifact, and validates JSONL logs under `{output_dir}`. Treat later
